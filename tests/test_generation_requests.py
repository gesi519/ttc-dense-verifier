...
class GenerationRequestTests(unittest.TestCase):
    ...
    def test_positive_prompt_requests_rigorous_answer(self):
        ...
        self.assertIn("test cases and expected outcomes", rendered)
        self.assertIn("do not write a full test harness", rendered)
        self.assertIn("Causal Mechanism", rendered)
        self.assertIn("sufficient buffer capacity", rendered)
        self.assertIn("no out-of-bounds access", rendered)
        self.assertIn("minimal, self-contained, and compilable", rendered)
        self.assertIn("close every Markdown code fence", rendered)
        self.assertIn('char buf[16] = "abcdef";', rendered)
        self.assertIn("never use `char buf[]", rendered)
        self.assertIn("do not themselves rely on undefined behavior", rendered)
        self.assertIn("that abstract guarantee belongs to `memmove`", rendered)
        self.assertIn("account for the null terminator", rendered)
        self.assertIn("unchanged suffix", rendered)
        self.assertIn("when `dest < src`, forward copying is safe", rendered)
        self.assertIn("when `dest > src`, backward copying is needed", rendered)
        self.assertIn("avoid calling runtime undefined behavior \"ill-formed\"", rendered)
        self.assertIn("pointer validity assumptions", rendered)
        self.assertIn("Limitations", rendered)
        self.assertIn("fully satisfies every output requirement", rendered)

...
tmux new-session -d -s ttc_generator_service "cd \"/data/lry_machine_learning/ttc_dense_verifier\"; CUDA_VISIBLE_DEVICES=\"${GENERATOR_CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5}\" vllm serve \"${GENERATOR_MODEL_PATH:?set GENERATOR_MODEL_PATH}\" --served-model-name \"${GENERATOR_MODEL:-Qwen2.5-32B-Instruct}\" --host \"${GENERATOR_HOST}\" --port \"${GENERATOR_PORT}\" --tensor-parallel-size \"${GENERATOR_TENSOR_PARALLEL_SIZE:-6}\" > \"outputs/logs/generator_service_${TTC_RUN_ID}.log\" 2>&1"
